...
export const DATA = [
  ...
  {
    id: "pro-8",
    title: "Смарт-часы Haylou RS4 PLUS Black",
    pirce: 255_000,
...
//           const getProduct = await getDocs(productRef)
//           setData( getProduct.docs.map(item => ({ ...item.data(), id: item.id }) ) )
      ...
      <div className="product__wrapper">
        {
          DATA?.map(item => ( <div className='product__card' >
            ...
            <i className='icon'><AiFillStar/><AiFillStar/><AiFillStar/><AiFillStar/><AiFillStar/> <FiMessageSquare className='icon__fi'/><p>0 ta sharh</p></i>
            ...
            {/* {
                            ...
                            } */}
              {
                heart?.some(i => i.id === item.id) ?
                  <AiFillHeart className='ikonka' style={{ color: "#7000FF" }} onClick={() => dispatch({ type: REMOVE_FROM_HEART, payload: item })} />
